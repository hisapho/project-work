...
$(document).ready(function(e) {
	...
	$('.og_next').click(function(){
		...
		if($('.mainlist li').length>4){//多于4张图片
			...
			swapLeftPos = parseInt($('.swaplist').css('left'));//交换图片ul位置
			if(ml<=0 && ml>w*-1){//默认图片显示时
				...
				$('.swaplist').animate({left: swapLeftPos - 1000 + 'px'},'slow');//交换图片滚动
				if(swapLeftPos==(w-1000)*-1){//交换图片最后一屏时
	...
	$('.og_prev').click(function(){
		...
		if($('.mainlist li').length>4){
			...
			swapLeftPos = parseInt($('.swaplist').css('left'));
			if(ml<=0 && ml>w*-1){
				...
				$('.swaplist').animate({left: swapLeftPos + 1000 + 'px'},'slow');
				if(swapLeftPos==0){
